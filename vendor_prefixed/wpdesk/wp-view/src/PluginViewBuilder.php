...
namespace CSVExporterVendor\WPDesk\View;

use CSVExporterVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use CSVExporterVendor\WPDesk\View\Resolver\ChainResolver;
use CSVExporterVendor\WPDesk\View\Resolver\DirResolver;
use CSVExporterVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \CSVExporterVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \CSVExporterVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \CSVExporterVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \CSVExporterVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
