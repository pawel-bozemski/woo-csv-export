...
if (!\class_exists('CSVExporterVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        {
            if (\class_exists('CSVExporterVendor\\WPDesk_Logger_Factory')) {
                \CSVExporterVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \CSVExporterVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \CSVExporterVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \CSVExporterVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\CSVExporterVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \CSVExporterVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
